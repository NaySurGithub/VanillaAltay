...
final class CrimsonFungiTreePopulator extends SeededPopulator{
	...
	public function populate(BlockManager $root, ChunkManager $world, int $chunkX, int $chunkZ, int $seed) : void{
		...
		for($i = 0; $i < $amount; ++$i){
			...
			foreach($this->getHighestWorkableBlocks($object, $x, $z) as $y){
				...
				HugeFungus::crimson($random->nextInt(9) + 4)->placeObject($object, $x, $y, $z, $random);
			}
		}
		$root->merge($object);
	}
	...
	private function getHighestWorkableBlocks(BlockManager $level, int $x, int $z) : array{
		$blockYs = [];
		for($y = 128; $y > 0; --$y){
			if($level->getBlockIdAt($x, $y, $z) === "minecraft:crimson_nylium" && $level->getBlockAt($x, $y + 1, $z)->canBeReplaced()){
				$blockYs[] = $y + 1;
			}
		}
		return $blockYs;
